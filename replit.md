...
A product catalog extraction pipeline dynamically generates categories from PDFs, supports fuzzy deduplication, product variants, and broadcasts `MENU_UPDATED` events via an in-process event bus. Admin endpoint `POST /api/admin/extract-products` allows manual re-extraction with dry-run and filter options; "Update Menu Catalog" menu item in admin Settings dropdown triggers extraction with spinner UI.
