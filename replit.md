...
A product catalog extraction pipeline dynamically generates categories from PDFs, supports fuzzy deduplication, product variants, and broadcasts `MENU_UPDATED` events via an in-process event bus. Admin endpoint `POST /api/admin/extract-products` allows manual re-extraction with dry-run and filter options; "Update Menu Catalog" menu item in admin Settings dropdown triggers extraction with spinner UI. Extraction is tuned for reliability: chunks are 3.5KB max (from 12KB for faster API calls), max_tokens reduced to 1500 for faster response times, 60-second timeout per call, 1 retry per failed chunk. Frontend shows product count on success and error details on failure.
ProductCatalog React component provides glassmorphism UI with category tabs, debounced search (300ms), product cards with detail modals, and a collapsible cart sidebar. Cart persists to localStorage (24h TTL) and features "Send to Chat" button for natural-language order submission. Audio cue `add-to-cart` (880Hz sine, 0.35s) plays when items are added. Fully responsive, WCAG-accessible, with React Query caching (5min staleTime).
...
Admin Product Management Interface (Prompt 30): Full CRUD at `/admin/products` via `client/src/pages/admin-products.tsx`. Backend REST endpoints (`GET/POST/PUT/DELETE /api/admin/products`, `POST /api/admin/products/upload-image`) protected by `requireAuth`. Image uploads use `multer` storing to `public/product-images/` with UUID filenames served via `/product-images/:filename`. Product deletions clean up local image files. All mutations emit `menu:updated` events. "Manage Products" link added to admin Settings dropdown; admin products page features debounced search, category/active filters, table with thumbnails, create/edit modal (name, description, price, category, variants, allergens, predefined tags, image upload, isActive toggle), and Run Extraction button.
