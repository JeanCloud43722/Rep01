...
- **Chat Soft Lock (Prompt 30.4 v1.0)**: `isConfirmationPending` state in GuestAssistant disables chat textarea and send button when order confirmation is pending; CartSidebar respects confirmation pending state via prop passed from parent; confirmation state tracked via useEffect that monitors order preview presence; OrderConfirmation calls `onResolved` callback to notify parent of completion (confirmed/cancelled); state flows from GuestAssistant → customer.tsx → ProductCatalog → CartSidebar; placeholder text guides users ("Please confirm or cancel your current order above."); accessible disabled states with aria-disabled and aria-describedby attributes; clear visual feedback (grayed out, cursor-not-allowed) for both light and dark themes.
...
- **Admin Dashboard Header Urgency (Prompt 31.1 v1.0)**: Header decluttered with Settings dropdown containing secondary actions (Refresh, Mute/Unmute, Clear Completed, Logout). Primary action "New Order" remains visible + prominent. Urgency-based sorting implemented via `useMemo` with hierarchy: unacknowledged service requests (score 0) → unread customer messages (score 1) → action-needed statuses (score 2) → done statuses (score 3) → time-based tiebreaker. Visual cues: red pulsing dot + red border for unacknowledged service requests; blue border + "Unread" badge for unread customer messages. Helper functions: `hasUnreadMessage()` (last message from customer), `hasUnackServiceRequest()` (any unacknowledged request). OrderCard accepts optional props `hasUnreadMsg` and `hasUnackSvcReq` for conditional styling.
