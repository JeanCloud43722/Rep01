...
class AudioManager {
  ...
  async unlock(): Promise<boolean> {
    if (this._isUnlocked && this.audioContext?.state === 'running') return true;
    ...
    try {
      ...
      // Mandatory user-gesture triggered silent playback to "prime" the context
      ...
      console.log('Audio Manager: audio unlocked via user gesture');
      this.notifyUnlockListeners();
      return true;
    } catch (error) {
  ...
  private cues: Record<SoundCue, AudioCueConfig> = {
    'order-ready': {
      ...
      play: (ctx, now) => {
        ...
        playChime(now, 880, 659.25);
        playChime(now + 0.7, 880, 659.25);
      }
    },
    'message': {
      duration: 0.6,
      play: (ctx, now) => {
        ...
        notes.forEach(({ freq, start }) => {
          this.createOscillator(ctx, 'sine', freq, now + start, 0.25, {
            attack: 0.02, sustain: 0.1, release: 0.13, peak: 0.3
          });
        });
      }
    },
    'offer': {
      ...
      play: (ctx, now) => {
        const notes = [
          { freq: 392, start: 0 },
          { freq: 493.88, start: 0.1 },
          { freq: 587.33, start: 0.2 },
          { freq: 783.99, start: 0.3 },
          { freq: 880, start: 0.4 },
        ];
        notes.forEach(({ freq, start }) => {
    ...
    'service-request': {
      ...
      play: (ctx, now) => {
        const playSequence = (offset: number) => {
          const tones = [
            { freq: 880, start: 0 },
            { freq: 698.46, start: 0.12 },
            { freq: 523.25, start: 0.24 },
          ];
          tones.forEach(({ freq, start }) => {
            this.createOscillator(ctx, 'triangle', freq, now + offset + start, 0.15, {
              attack: 0.01, sustain: 0.08, release: 0.06, peak: 0.35
            });
          });
        };
        playSequence(0);
      ...
      play: (ctx, now) => {
        ...
        notes.forEach(({ freq, start }) => {
          this.createOscillator(ctx, 'sine', freq, now + start, 0.2, {
            attack: 0.02, sustain: 0.08, release: 0.1, peak: 0.25
          });
        });
      }
    },
    'order-completed': {
      ...
      play: (ctx, now) => {
        this.createOscillator(ctx, 'sine', 349.23, now, 0.2, {
          attack: 0.02, sustain: 0.1, release: 0.08, peak: 0.3
        });
        this.createOscillator(ctx, 'sine', 523.25, now + 0.2, 0.25, {
          attack: 0.02, sustain: 0.12, release: 0.11, peak: 0.3
        });
      }
    }
  };
...
export function useAudioManager() {
  return {
    warmUp: () => audioManager.warmUp(),
    unlock: () => audioManager.unlock(),
    isUnlocked: () => audioManager.isUnlocked,
    play: (cue: SoundCue) => audioManager.play(cue),
    playIfUnlocked: (cue: SoundCue) => audioManager.playIfUnlocked(cue),
    setVolume: (level: number) => audioManager.setVolume(level),
